...
def create_and_fire_query_WhenWhere(line):
	...
	for token in line:
		...
		if (token.dep_ == "compound" or token.dep_ == "amod" or token.dep_ == "nmod" or token.dep_ == "advcl") and token.head.dep_ == "nsubj" :
			subject.append(token.lemma_)
		if token.dep_ == "ROOT":
		...
		if (token.dep_ == "appos" and token.head.dep_ == "nsubj") or (token.dep_ == "compound" and token.head.dep_ == "appos"):
